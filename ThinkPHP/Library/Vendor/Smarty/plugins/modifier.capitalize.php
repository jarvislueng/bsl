    ...
    if (SMARTY_MBSTRING /* ^phpunit */&&empty($_SERVER['SMARTY_PHPUNIT_DISABLE_MBSTRING'])/* phpunit$ */) {
        if ($lc_rest) {
            ...
            $upper_string = preg_replace("!(^|[^\p{L}'])([\p{Ll}])!ueS", 
                "stripslashes('\\1').mb_convert_case(stripslashes('\\2'),MB_CASE_UPPER, SMARTY_RESOURCE_CHAR_SET)", $string);
        }
        ...
        $upper_string = preg_replace("!((^|\s)['\"])(\w)!ue", 
            "stripslashes('\\1').mb_convert_case(stripslashes('\\3'),MB_CASE_UPPER, SMARTY_RESOURCE_CHAR_SET)", $upper_string);
        return $upper_string;
    }
