...
class Template_Lite_Compiler extends Template_Lite {
    ...
    function _compile_file($file_contents) {
        ...
        $file_contents = preg_replace("!{$ldq}\s*literal\s*{$rdq}(.*?){$ldq}\s*/literal\s*{$rdq}!s", stripslashes($ldq . "literal" . $rdq), 
            $file_contents);
        ...
        foreach ($this->_require_stack as $key=>$value) {
            $compiled_text = '<?php require_once(\'' . $this->_get_plugin_dir($key) . $key . '\'); $this->register_' . $value[0] . '("' . $value[1] .
                 '", "' . $value[2] . '"); ?>' . $compiled_text;
        }
    ...
    function _compile_tag($tag) {
        ...
        preg_match_all(
            '/(?:(' . $this->_var_regexp . '|' . $this->_svar_regexp . '|\/?' . $this->_func_regexp . ')(' . $this->_mod_regexp .
                 '*)(?:\s*[,\.]\s*)?)(?:\s+(.*))?/xs', $tag, $_match);
        
        if ($_match[1][0]{0} == '$' || ($_match[1][0]{0} == '#' && $_match[1][0]{strlen($_match[1][0]) - 1} == '#') || $_match[1][0]{0} == "'" ||
             $_match[1][0]{0} == '"' || $_match[1][0]{0} == '%') {
    ...
    function _parse_function($function, $modifiers, $arguments) {
        switch ($function) {
            ...
            case 'foreach' :
                ...
                $_result = '<?php if (count((array)' . $_args['from'] . ')): foreach ((array)' . $_args['from'] . ' as ' . $_args['key'] .
                     '$this->_vars[\'' . $_args['value'] . '\']): ?>';
                return $_result;
                break;
            case 'foreachelse' :
            ...
            case 'for' :
                ...
                $_result = '<?php for($for' . $this->_for_stack . ' = ' . $_args['start'] . '; ((' . $_args['start'] . ' < ' . $_args['stop'] .
                     ') ? ($for' . $this->_for_stack . ' < ' . $_args['stop'] . ') : ($for' . $this->_for_stack . ' > ' . $_args['stop'] . ')); $for' .
                     $this->_for_stack . ' += ((' . $_args['start'] . ' < ' . $_args['stop'] . ') ? ' . $_args['step'] . ' : -' . $_args['step'] .
                     ')): ?>';
                if (isset($_args['value'])) {
            ...
            switch ($state) {
                ...
                case 2 :
                    if ($value != '=') {
                        ...
                        if (! preg_match_all(
                            '/(?:(' . $this->_var_regexp . '|' . $this->_svar_regexp . ')(' . $this->_mod_regexp . '*))(?:\s+(.*))?/xs', $value, 
                            $_variables)) {
    ...
    function _compile_variable($variable) {
        ...
        if ($var_name == $this->reserved_template_varname) {
            if ($variable[0]{0} == '[' || $variable[0]{0} == '.') {
                ...
                $this->trigger_error('$' . $var_name . implode('', $variable) . ' is an invalid $templatelite reference', E_USER_ERROR, __FILE__, 
                    __LINE__);
            }
        } else {
                ...
                if ($var{0} == '.') {
                    ...
                    if (substr($var, 0, 2) == '->') {
                        if (substr($var, 2, 2) == '__') {
                            ...
                            if (substr($var, 2, 1) == '$') {
                                $_output .= '->{(($var=$this->_TPL[\'' . substr($var, 3) .
                                     '\']) && substr($var,0,2)!=\'__\') ? $_var : $this->trigger_error("cannot access property \\"$var\\"")}';
                            }
                    } else {
    ...
    function _plugin_exists($function, $type) {
        ...
        if (isset($this->_plugins[$type][$function]) && is_array($this->_plugins[$type][$function]) && is_object($this->_plugins[$type][$function][0]) &&
