...
function compile_compile_if($arguments, $elseif, $while, &$object) {
    ...
    preg_match_all(
        '/(?>(' . $object->_var_regexp . '|\/?' . $object->_svar_regexp . '|\/?' . $object->_func_regexp . ')(?:' . $object->_mod_regexp .
             '*)?|\-?0[xX][0-9a-fA-F]+|\-?\d+(?:\.\d+)?|\.\d+|!==|===|==|!=|<>|<<|>>|<=|>=|\&\&|\|\||\(|\)|,|\!|\^|=|\&|\~|<|>|\%|\+|\-|\/|\*|\@|\b\w+\b|\S+)/x', 
            $arguments, $_match);
    $_args = $_match[0];
    ...
    for($i = 0, $for_max = $count_args; $i < $for_max; $i ++) {
        ...
        switch (strtolower($_arg)) {
            ...
            default :
                ...
                if (isset($_match[0]{0}) &&
                     ($_match[0]{0} == '$' || ($_match[0]{0} == '#' && $_match[0]{strlen($_match[0]) - 1} == '#') || $_match[0]{0} == "'" ||
                     $_match[0]{0} == '"' || $_match[0]{0} == '%')) {
                    // process a variable
                    ...
                    // pass the number through
                     ...
                     strtolower($_match[0]) == "true" || strtolower($_match[0]) == "false" || strtolower($_match[0]) == "null") {
