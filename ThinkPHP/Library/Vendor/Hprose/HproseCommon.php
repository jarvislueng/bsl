...
class HproseTime {
    ...
    public static function isValidTime($hour, $minute, $second, $microsecond = 0) {
        return ! (($hour < 0) || ($hour > 23) || ($minute < 0) || ($minute > 59) || ($second < 0) || ($second > 59) || ($microsecond < 0) ||
             ($microsecond > 999999));
    }
}
class HproseDateTime extends HproseDate {
    ...
    public function equals($when) {
        ...
        return (($this->year == $when->year) && ($this->month == $when->month) && ($this->day == $when->day) && ($this->hour == $when->hour) &&
             ($this->minute == $when->minute) && ($this->second == $when->second) && ($this->microsecond == $when->microsecond));
    }
    public function timestamp() {
        if ($this->utc) {
