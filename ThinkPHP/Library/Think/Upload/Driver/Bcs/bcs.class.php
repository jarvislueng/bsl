...
class BaiduBCS {
    ...
    private function authenticate($opt) {
        ...
        if (! ('/' == $opt[self::OBJECT] && '' == $opt[self::BUCKET] && 'GET' == $opt[self::METHOD] &&
             ! isset($opt[self::QUERY_STRING][self::ACL])) && ! self::validate_bucket($opt[self::BUCKET])) {
    ...
    public function create_object($bucket, $object, $file, $opt = array()) {
        ...
        $response = $this->authenticate($opt);
        $this->log(
            $response->isOK() ? "Create object[$object] file[$file] success!" : "Create object[$object] file[$file] failed! Response: [" .
                 $response->body . "] Logid[" . $response->header["x-bs-request-id"] . "]", $opt);
        return $response;
    }
    ...
    public function create_object_by_content($bucket, $object, $content, $opt = array()) {
        ...
        $response = $this->authenticate($opt);
        $this->log(
            $response->isOK() ? "Create object[$object] success!" : "Create object[$object] failed! Response: [" . $response->body . "] Logid[" .
    ...
    public function create_object_superfile($bucket, $object, $file, $opt = array()) {
        ...
        for($i = 0; $i < $sliceNum; $i ++) {
            ...
            $this->log(
                "Begin to upload Sub-object[" . $opt[self::OBJECT] . "][$i/$sliceNum][seekto:" . $opt['seekTo'] . "][Length:" . $opt['length'] . "]", 
                $opt);
            $response = $this->create_object($bucket, $opt[self::OBJECT], $file, $opt);
            if ($response->isOK()) {
                $this->log(
                    "Sub-object upload[" . $opt[self::OBJECT] . "][$i/$sliceNum][seekto:" . $opt['seekTo'] . "][Length:" . $opt['length'] .
                         "]success! ", $opt);
            ...
            if (is_int($opt["seek_object_id"]) && $opt["seek_object_id"] <= $objectCount) {
                foreach ($objects as $object=>$file) {
                    if ($num < $opt["seek_object_id"]) {
                        $this->log(
                            "Seeking object of [" . $opt["seek_object_id"] . "/$objectCount]. Skip  id[$num/$objectCount]object[$object]file[$file].", 
                            $opt);
                        // $result ['skipped'] [] = "[$num/$objectCount] " . $file;
                        $result['skipped'] ++;
                        unset($objects[$object]);
    ...
    public function copy_object($source, $dest, $opt = array()) {
        ...
        if (empty($dest) || ! is_array($dest) || ! isset($dest[self::BUCKET]) || ! isset($dest[self::OBJECT]) ||
             ! self::validate_bucket($dest[self::BUCKET]) || ! self::validate_object($dest[self::OBJECT])) {
