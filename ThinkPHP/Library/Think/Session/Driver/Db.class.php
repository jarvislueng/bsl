...
class Db {
    ...
    public function open($savePath, $sessName) {
        ...
        if (1 == C('DB_DEPLOY_TYPE')) {
            ...
            if (C('DB_RW_SEPARATE')) {
                ...
                // 主数据库链接
                $hander = mysql_connect($host[$w] . (isset($port[$w]) ? ':' . $port[$w] : ':' . $port[0]), isset($user[$w]) ? $user[$w] : $user[0], 
                ...
                // 从数据库链接
                $hander = mysql_connect($host[$r] . (isset($port[$r]) ? ':' . $port[$r] : ':' . $port[0]), isset($user[$r]) ? $user[$r] : $user[0], 
    ...
    public function write($sessID, $sessData) {
        ...
        mysql_query('REPLACE INTO  ' . $this->sessionTable .
