...
<body class="default-bg-color">
	...
	<div style="float: left;">
							<form class="form-inline" role="form"
								id="<?php echo ($tablename); ?>_search_form">
			<?php if(is_array($toolbar_form_param)): foreach($toolbar_form_param as $key=>$toolbar): if($toolbar['type'] == 1): ?><div
									class="form-group" style="float: left; margin-right: 20px;">
									<label><?php echo ($toolbar['field_title']); ?> </label> <input type="text"
					name="<?php echo ($toolbar['field_name']); ?>" id="<?php echo ($toolbar['field_id']); ?>"
					class="form-control" style="width: auto; margin-left: 5px; <?php echo ($toolbar['style']); ?>"
			...
			<div class="form-group" style="float: left; margin-right: 20px;">
					...
					class="form-control" style="width: auto; margin-left: 5px; <?php echo ($toolbar['style']); ?>">
					<?php if(is_array($toolbar['option'])): foreach($toolbar['option'] as $key=>$options): if($toolbar['default_value'] == $options['key']): ?><option
											value="<?php echo ($options['key']); ?>" selected="true"><?php echo ($options['value']); ?></option>
					<?php else: ?>
	...
	<?php } ?>
	<?php if(is_array($toolbar_btn_param)): foreach($toolbar_btn_param as $key=>$toolbar_btn): ?><div
							style="float: left;">
							<button type="button"
								id="<?php echo ($toolbar_btn['field_id']); ?>"
